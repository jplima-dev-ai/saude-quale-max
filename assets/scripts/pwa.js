(() => {
  ...
  const paginaProduto = location.pathname.includes("/products/");
  ...
  const obterConfig = () => {
    if (window.QualimaxConfig) return Promise.resolve(window.QualimaxConfig);
    if (promessaConfig) return promessaConfig;

    promessaConfig = new Promise((resolve) => {
      let concluida = false;
      const concluir = (config = {}) => {
        if (concluida) return;
        concluida = true;
        window.clearTimeout(timer);
        document.removeEventListener("qualimax:config-ready", aoCarregarConfig);
        resolve(config || {});
      };
      const aoCarregarConfig = (evento) => concluir(evento.detail || window.QualimaxConfig || {});
      const timer = window.setTimeout(() => concluir(window.QualimaxConfig || {}), 1400);
      document.addEventListener("qualimax:config-ready", aoCarregarConfig, { once: true });
      fetch(`${raiz}data/config.json`, { cache: "no-cache" })
  ...
  if ("serviceWorker" in navigator && /^https?:$/.test(location.protocol)) {
    ...
    window.addEventListener("load", async () => {
      ...
      try {
        registroSW = await navigator.serviceWorker.register(`${raiz}service-worker.js`, { scope: raiz });
